...
export class ColorMapEditor extends Container {
    ...
    setInterpolationMethods(interpolationMethod) {
     ...
     */
    setUpInputElements(finalOptions) {
        ...
        if (finalOptions.binSelectorEditable) {
            ...
            this.binsElement.addEventListener('wheel', (ev) => {
                ev.preventDefault();
            ...
            this.binsElement.addEventListener('keypress', (ev) => {
                if (ev.key === 'Enter') {
                    validateBins(ev);
                }
            });
